...
uv run researchd pilot --project-id <id> --import-decision D-001=A   # 创建 pilot
uv run pytest -q                   # 134 passed + 2 门控跳过
```
...
| `docs/completion-report.md` | v0.1 完成报告（§30 格式） |
...
| `docs/blockers.md` | 真实 blocker（B-01 飞书、B-03 付费模型、B-06 cc-connect、B-07 systemd） |
| `docs/pilot.md` | Pilot 定义与 bootstrap |
...
src/researchd/
  ...
  projections/  飞书文档投影（增量/hash 收敛/PI Notes 保护）
  api/          UDS/TCP 内部 API（入站幂等、授权、409）
  delivery/     投递端口（FakeDeliveryPort；真实平台 B-01）
  ops/          备份/恢复/导出
  cli.py        入口：researchd service/acp/migrate/pilot/backup/restore/export
  ctl.py        researchctl（doctor/交互）
migrations/     Alembic
deploy/         systemd unit + env 模板
integrations/   cc-connect 窄补丁（Delivery API）
...
- **真实平台 GATED**：飞书（B-01）与真实付费模型（B-03）需宿主授权后才启用；
  当前生产配置 `executor=fake`、`delivery=fake`、`doc_platform=none`。
